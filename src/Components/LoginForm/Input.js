...
import {mergeClassName, prepareElementClassName} from './FormLogic';
...
const Input = ({ fieldName, label, value, onChange, type = "text", onValidate, className }) => {
  ...
  return (
    <div 
        className={mergeClassName(prepareElementClassName(className, "input-with-error_container"), isModified && isInvalid ? "invalid" : "valid")}>
